...
# Each service gets:
#   - its own database
#   - a DDL account (used by Flyway only) with CREATE privileges on the schema
#   - an app account (used by the running service) with CRUD only
#   - default privileges so the app account auto-inherits access to tables
#     created by the DDL account
...
SQL
  ...
  # Always sync password — safe to re-run, handles rotation
  psql_admin postgres -c "ALTER USER $ddl_user WITH PASSWORD '$ddl_password';" > /dev/null

  # App account (running service)
...
SQL
  ...
  # Schema privileges — run against the service DB.
  # DDL account gets CREATE so Flyway can create/alter/drop objects.
  # App account gets USAGE only.
  # Grant on ALL TABLES/SEQUENCES covers any objects that predate this script
  # (e.g. flyway_schema_history created by a previous superuser).
  # Default privileges cover future objects created by the DDL account.
