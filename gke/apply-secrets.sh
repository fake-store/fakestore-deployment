#!/usr/bin/env bash
# Reads secrets from secrets.env and applies them to the GKE cluster. Idempotent.
#
...
# Optional: create imagePullSecret for private ghcr.io packages
if [[ -n "${GHCR_TOKEN:-}" && -n "${GHCR_USERNAME:-}" ]]; then
  ...
  echo "  [ok] ghcr-secret (private image pull)"
fi
